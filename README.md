...
- `EMIT-<YYYYMMDD>-chla.tif`
- `EMIT-<YYYYMMDD>-tss.tif`
- `EMIT-<YYYYMMDD>-acdom.tif`
- `EMIT-<YYYYMMDD>-products.nc`

The date is parsed from the input filename. When several EMIT passes share a
...
runs ACOLITE + inference, and publishes the resulting GeoTIFFs to two places:

- the repository's **`EMIT-Data`** release
- the **Hugging Face dataset** (under `cogs/`):
  https://huggingface.co/datasets/giswqs/EMIT-Water-Quality
